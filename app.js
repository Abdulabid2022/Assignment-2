...
let init = () => {
  ...
  keysArr.map(
    (x) => (dropdown.innerHTML += `<option value='${x}'>${x}</option>`)
  );
};
let searchVal = () => {
  ...
  // console.log(dropdownValue, inpValue);
  let filterData = data.filter((x) =>
    typeof x[dropdownValue] == "number"
      ? x[dropdownValue] == inpValue
      : x[dropdownValue].includes(inpValue)
  );
